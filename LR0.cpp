...
void findclosure(int z, char a)
{
    ...
    // Check for items in the closure and add them to the new closure set
    for (i = 0; i < arr[z]; i++)
    {
    ...
    // Find additional items to be added to the closure set
    for (i = 0; i < n; i++)
    {
    ...
    // Store the size of the closure set in the array
    arr[noitem] = n;

    int flag = 0;

    // Check if the new closure set already exists
    for (i = 0; i < noitem; i++)
    {
        if (arr[i] == n)
        {
...
// Parse the productions to create the grammar
for (n = 0; n < i - 1; n++)
{
    m = 0;
    j = novar;
...
// Find the closure for each item and build the set of items
